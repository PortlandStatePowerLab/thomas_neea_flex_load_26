...
Recovered filter logic:
  ...
  - in.hvac_heating_type_and_fuel = "Electricity ASHP" OR "Electricity MSHP"
  - 
...
Modified by Thomas Metzler for Heating 7/23/26
"""
...
def recreate_files(source_file: Path = SOURCE_FILE, output_dir: Path = OUTPUT_DIR) -> None:
    ...
    base_filter = (
        ...
        & ((df["in.hvac_heating_type_and_fuel"] == "Electricity ASHP") | (df["in.hvac_heating_type_and_fuel"] == "Electricity MSHP"))
        & (~df["bldg_id"].isin(EXCLUDE_BLDG_IDS))
    )

    df[base_filter].to_csv (output_dir / 'Heat_OR.csv', index=False)
