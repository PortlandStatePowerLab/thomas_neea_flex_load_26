...
def get_ev_charger_power(hpxml_path, default_kw=20):
    try:
        ...
        print(f"[WARNING] Failed to parse EV charger level. Using default. Error: {e}")
    return default_kw
...
def get_ev_capacity_or_range(hpxml_path, default_capacity_kwh=60.0):
    try:
        ...
        print(f"[WARNING] Failed to parse EV capacity. Using default. Error: {e}")
...
def determine_EV_control(global_mode, control_mode, charger_kw, ev_name):
    ctrl_signal = {ev_name: {}}

    if global_mode == 'NORMAL':
        fraction = 1.0
    elif global_mode == 'SHED':
        fraction = SHED_PCT
    else:
        fraction = 1.0 # Failsafe
...
def init_fleet_worker(home):
    ...
    base_dw, sim_dw, charger_kw, primary_ev = initialize_home(home, WEATHER_FILE)
    return {
        "base": base_dw, 
        "sim": sim_dw, 
        "path": home,
...
if __name__ == "__main__":
    ...
    for sim_time in sim_times:
        ...
        if is_vpp_active:
            ...
            if pid_output < -AVERAGE_DEADBAND_KW:
                ...
                # Verify EV is actually charging before attempting to shed it
        ...
        for home_data in fleet_data:
            ...
            control_cmd = determine_EV_control(
                global_mode=home_data["override"], 
                control_mode=CONTROL_MODE, 
                charger_kw=home_data["charger_kw"], 
                ev_name=home_data["primary_ev"]
            )
    ...
    for home_data in fleet_data:
        ...
        final_ctrl_cols = ["Time", "Total Electric Power (kW)", "Total Electric Energy (kWh)", "EV Electric Power (kW)", "EV SOC (-)"]
        final_base_cols = ["Time", "Total Electric Power (kW)", "Total Electric Energy (kWh)", "EV Electric Power (kW)", "EV SOC (-)"]
        
        df_ctrl = df_ctrl[[c for c in final_ctrl_cols if c in df_ctrl.columns]]
        df_base = df_base[[c for c in final_base_cols if c in df_base.columns]]
