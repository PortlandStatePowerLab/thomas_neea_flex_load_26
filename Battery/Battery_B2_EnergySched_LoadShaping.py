...
filename = 'Battery_Test_LoadShape_0.5_0.2_0.1'
...
AVERAGE_SETPOINT_KW = 1.2     # Target average power PER HOME during VPP event
AVERAGE_DEADBAND_KW = 0.1     # Tolerance PER HOME to prevent constant toggling
ESTIMATED_LOAD_KW = P_LOAD_UP_KW       # Est. power ADDED when forcing a unit ON (LOAD) or lost when restored to NORMAL
ESTIMATED_SHED_KW = -P_SHED_KW         # Est. power DROPPED when allowing a unit to SHED or gained when restored to NORMAL
...
def determine_battery_control(global_mode="NORMAL"):
    """
...
if __name__ == "__main__":
    ...
    for sim_time in sim_times:
        ...
        if is_vpp_active:
            ...
            max_fleet_kw = num_homes * P_LOAD_UP_KW
            integral_error = max(-max_fleet_kw, min(integral_error, max_fleet_kw))
        ...
        for home_data in fleet_data:
           ...
           # 2. Controlled Update (driven purely by the VPP state now)
            control_cmd = determine_battery_control(global_mode=home_data["override"])
