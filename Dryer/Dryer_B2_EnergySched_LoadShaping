...
def determine_hvac_control(global_mode="NORMAL"):
    ...
    if global_mode == "SHED":
        ctrl_signal['HVAC Heating'].update({'Setpoint': Tcontrol_SHEDC})
    elif global_mode == "LOAD":
        ctrl_signal['HVAC Heating'].update({'Setpoint': Tcontrol_LOADC})
...
if __name__ == "__main__":
    ...
    CTRL_COLS = [
        ...
        "HVAC Heating Electric Power (kW)", "HVAC Heating COP (-)",
        "Temperature - Indoor (C)", "HVAC Cooling Electric Power (kW)" 
    ]
